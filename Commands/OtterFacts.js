...
class OtterFacts extends Command {
  ...
  run() {
    if ((this.getArguments()[1] != null) && this.getArguments()[1].toString().toLowerCase() === "description") {
      // send fact + description in quote form
      this.sendQuote(this.getFact() + "\n\n" + this.getDescription());
      this.lastFact = this.factNum;
      this.factNum++;
    }
    else if((this.getArguments()[1] != null) && this.getArguments()[1].toString().toLowerCase() === "last") {
